...
Filename: languageTranslater-tRaj-00.py
Author: Tushar Raj
...
while True: #Starting the while loop
    ...
    if(userInput_lower in english): #checking if it is english word or not
        ...
        if(addWord.lower() == "y"): #checking if user want to add word in list if it present or not
            ...
            if(lang.lower() == "e"): 
                lang_french = input(f"what is the French word for '{userInput_check}'? ")
                ...
                lang_english = input(f"what is the french word for '{userInput_check}'? ")
